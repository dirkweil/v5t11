...
public class AbsVerticalSeekBar extends VerticalProgressBar
{
  ...
  void onProgressRefresh(float scale, boolean fromUser)
  {
    ...
    if (thumb != null)
    {
      ...
      /*
       * Since we draw translated, the drawable's bounds that it signals for invalidation won't be the actual bounds we want
       * invalidated, so just invalidate this whole view.
       */
      invalidate();
    }
  }
  ...
  protected void onSizeChanged(int w, int h, int oldw, int oldh)
  {
    ...
    if (thumbWidth > trackWidth)
    {
      ...
      if (d != null)
      {
        ...
        d.setBounds(gapForCenteringTrack, 0, w - mPaddingRight - mPaddingLeft - gapForCenteringTrack, h - mPaddingBottom - mPaddingTop);
      }
    }
    else
    {
