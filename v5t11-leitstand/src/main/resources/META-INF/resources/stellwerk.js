...
function updateHtmlElement(drawCommand) {
  ...
  if (htmlElement != null) {
    ...
    drawSignal(drawCommand.s, drawCommand.p, drawCommand.l, ctx);
  }
}
...
function drawInaktiv(inaktiveRichtungen, ctx) {
  if (inaktiveRichtungen != null) {
    ...
    for (inaktiveRichtung of inaktiveRichtungen) {
      const start = richtung2point(inaktiveRichtung);
...
function drawAktiv(aktiveRichtungen, besetzt, ctx) {
  if (aktiveRichtungen != null) {
    ...
    const start = richtung2point(aktiveRichtungen[0]);
...
function drawSignal(signal, position, lichter, ctx) {
  if (signal != null) {
    ctx.setTransform(1, 0, 0, 1, 0, 0);
    ctx.lineWidth = 40;
