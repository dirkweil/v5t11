...
- `systemControl` (`SystemControlPresenter`, 350 Zeilen) → `SystemControlView`; aktuiert echte Hardware, erhöhter Testaufwand. **Umgesetzt und vom Nutzer verifiziert ("funktioniert") — Detailplan/Ergebnis siehe Phase 1a unten.**
...
### Phase 1a — `SystemControlView` (Detailplan)

**Befund aus der Analyse:** Anders als `SystemStatusPresenter` (JSF-frei, `@Model`, direkt wiederverwendbar) ist `SystemControlPresenter` `@Named @SessionScoped` und importiert `FacesContext`/`FacesMessage` direkt in den Settern (Validierungsfeedback bei unbekanntem Gleis/Weiche/Signal bzw. ungültiger Fahrstufe). Er hält außerdem mutable Auswahl-State (Bereich/Gleis/Weiche/Signal/Lok) sessionweit. **Entscheidung (bestätigt):** `SystemControlPresenter` und `systemControl.xhtml` bleiben unangetastet; `SystemControlView` ruft die Fachklassen (`Steuerung`, `Zentrale`, `Gleis`, `Weiche`, `Signal`, `Fahrzeugdecoder`) direkt auf, mit eigenem View-lokalem State (Felder auf der View-Instanz, analog zum bisherigen `@PostConstruct`/`resetBereich()`-Pattern, da Vaadin-Route-Komponenten pro Navigation neu instanziiert werden). Validierungsfeedback über `com.vaadin.flow.component.notification.Notification` statt `FacesMessage`. **Verhalten wird 1:1 übernommen (bestätigt):** keine Confirm-Dialoge, jede Aktion wirkt sofort, exakt wie im JSF-Original — inkl. der Disabled-Logik (`zentraleConnected`/`gleisBesetzt` deaktiviert wenn `zentraleEchtbetrieb`) und der Nicht-blockierenden Validierung (unbekannter Name/ungültige Fahrstufe → Notification, kein Abbruch/Exception).

**Struktur/Layout** (`de.gedoplan.v5t11.status.vaadinui.SystemControlView`, `@Route(value = "system-control", layout = MainLayout.class)`, `@PageTitle("System-Control - v5t11")`), gruppiert wie die drei Fieldsets im Original:
1. **Allgemein:** Connected-Toggle (disabled wenn `zentraleEchtbetrieb`) + Gleisspannung-Toggle.
2. **Fahrweg:** Bereich-Auswahl (löst Reload von Gleis/Weiche/Signal-Panel aus) → Gleis (Name + Besetzt-Toggle, disabled wenn `zentraleEchtbetrieb`), Weiche (Name + Stellungs-Auswahl), Signal (Name + Stellungs-Auswahl + Button "alle: H" → `setAlleSignaleHalt()`-Äquivalent).
3. **Lok:** editierbare Auswahl (`DecoderAdr`, Format `adresse@systemTyp` — kein JSF-`Converter` nötig, stattdessen Item-Label-Generator), Aktiv-Toggle, Fahrstufe (Eingabefeld + Stellelement, Range 0..`lokMaxFahrstufe`) + Rückwärts-Toggle, Licht-Toggle, 16 Funktionsbits (F1–F16) als Toggle-Grid über `lok.getFktBits()`.

Komponenten-Mapping PrimeFaces → Vaadin (pro Element während der Umsetzung konkretisieren): `selectBooleanButton` → `Checkbox`/`ToggleButton`-Äquivalent, `selectOneMenu` → `ComboBox`/`Select`, `selectOneButton` (Weiche/Signal-Stellung) → `RadioButtonGroup`, Fahrstufen-Slider → zu klären (Vaadin Flow hat keinen eingebauten Slider-Core-Component; `IntegerField` + ggf. externe/Lit-Komponente prüfen).

**Live-Updates (neu zu bauen, war in Phase 0 nur als Idee skizziert):** Die JSF-Seite lädt bei jeder `@Changed`-CDI-Event (gefeuert über `EventFirer` für `Fahrzeugdecoder`/`Gleis`/`Signal`/`Weiche`/`Zentrale`, aktuell konsumiert von `PushService`/`AbstractPushService` und trigger ein komplettes Form-Reload). Für Vaadin: ein neuer, schlanker CDI-Bean `VaadinChangePushBroadcaster` (`@ApplicationScoped`) in `v5t11-status` beobachtet dieselben `@ObservesAsync @Changed`-Events und hält eine `CopyOnWriteArrayList<Consumer<Object>>` an Listenern (exakt das Muster, das `NavigationPresenter.menuChangeListeners` bereits für das Menü etabliert hat). `SystemControlView` registriert in `onAttach()` einen Listener, der zielgerichtet (nicht die ganze Seite) über `getUI().ifPresent(ui -> ui.access(...))` die betroffenen Felder aktualisiert, und deregistriert ihn in `onDetach()`. Das ersetzt den bisherigen "alles neu laden"-Ansatz durch gezielte Updates, ohne die bestehende JSF-Push-Infrastruktur (`PushService`) anzufassen.

**NavigationProducer:** `"System-Control (alt)"` existiert bereits (order 820). Neuer Eintrag `"System-Control"` → `this.urlPrefix + "ui/system-control"`, order knapp darunter (z. B. 815), gleiches Schema wie bei System-Status.

**Exit-Kriterien:** Build grün; `system-control`-Route neben JSF-Original erreichbar; alle Fahrweg-/Lok-Aktionen lösen dieselbe Fachlogik aus wie im JSF-Original (gleiche Methoden auf `Steuerung`/`Zentrale`/`Gleis`/`Weiche`/`Signal`/`Fahrzeugdecoder`); Live-Update-Mechanismus verifiziert (Änderung von außen — z. B. Sensor-Feedback oder zweite Session — erscheint ohne manuellen Reload); manueller Hardware-Smoke-Test (oder Dummy-Zentrale) durch den Nutzer vor Cutover, da diese View echte Hardware aktuiert.

**Status: erledigt.** `SystemControlView` (`v5t11-status/.../vaadinui/SystemControlView.java`) und `VaadinChangePushBroadcaster` (`.../webui/VaadinChangePushBroadcaster.java`, neuer `@ApplicationScoped`-CDI-Bean, beobachtet dieselben `@ObservesAsync @Changed`-Events wie `PushService` und verteilt sie an registrierte View-Listener nach dem `NavigationPresenter.menuChangeListeners`-Muster) sind angelegt, Reactor-Build grün, `NavigationProducer`-Eintrag `"System-Control"` (`ui/system-control`, order 815) ergänzt. Vom Nutzer in seiner Dev-Umgebung verifiziert ("funktioniert") — Toggles, Weichen-/Signalstellung, "alle: H", Lok-Auswahl/-Steuerung (inkl. Neuanlage per `adresse@systemTyp`) und Live-Updates arbeiten wie im JSF-Original. `SystemControlPresenter`/`systemControl.xhtml` unverändert als Rollback-Pfad erhalten.
...
- `/opt/prj/gedoplan/v5t11/v5t11-status/src/main/java/de/gedoplan/v5t11/status/vaadinui/SystemControlView.java` (neu anzulegen, Phase 1a)
- `/opt/prj/gedoplan/v5t11/v5t11-status/src/main/java/de/gedoplan/v5t11/status/webui/VaadinChangePushBroadcaster.java` (neu anzulegen, Phase 1a)
- `/opt/prj/gedoplan/v5t11/v5t11-status/src/main/java/de/gedoplan/v5t11/status/webui/SystemControlPresenter.java` (nur als Referenz, bleibt unverändert)
